...
class _VerCertificadosScreenState extends State<VerCertificadosScreen> {
  ...
  // ✅ OPT: flags para evitar doble tap en Ver / Descargar
  ...
  Future<void> _cargarCertificados() async {
    ...
    try {
      ...
      if (userData == null) {
        if (mounted) {
          setState(() {
            _error     = 'No se pudo obtener los datos del usuario.';
            _isLoading = false;
          });
        }
        return;
      ...
      if (carreraPath.isEmpty || studentId.isEmpty) {
        if (mounted) {
          setState(() {
            _error     = 'No se encontró la información académica del estudiante.';
            _isLoading = false;
          });
        }
        return;
  ...
  // ✅ OPT: _procesando evita que el usuario pulse Ver/Descargar
  //         dos veces seguidas mientras se genera el PDF
  ...
  Widget _buildBody() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        // ✅ OPT: +2 para el resumen (índice 0) y el spacer final (índice N+1)
  ...
  Widget _buildResumen() {
    // ✅ OPT: cálculo de roles solo cuando se construye el resumen,
    //         no en cada build() de la lista completa
  ...
  Widget _buildCertificadoCard(_CertificadoItem cert) {
    ...
    return Card(
      key: ValueKey(cert.id), // ✅ OPT: key para reconciliación eficiente
      elevation: 2,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    // ✅ OPT: Color con alpha precalculado en hex
            ...
            if (cert.datos.rol == 'ASISTENTE' &&
                cert.datos.horas.isNotEmpty) ...[
              const SizedBox(height: 4),
